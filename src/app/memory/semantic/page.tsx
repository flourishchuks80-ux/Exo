...
export default function SemanticMemoryPage() {
  ...
  const { data: memories, isLoading } = useSemanticMemory(walletAddress, masterKey);
  const deleteMutation = useDeleteMemory(walletAddress);

  const filtered = (memories ?? []).filter((m) =>
    search
      ? m.payload.content.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <main className="pt-14 lg:pl-56">
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-[#F0F4FF]">Semantic Memory</h1>
              <p className="text-sm text-[#8B9CC8] mt-1">
                {memories?.length ?? 0} facts stored on Arkiv Braga
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="text-[#8B9CC8] text-sm mb-4">No semantic memories yet.</p>
              <Link href="/memory/new">
                <Button variant="primary" size="sm" className="gap-1.5">
                  <Plus className="w-3.5 h-3.5" />
                  Add your first semantic memory
                </Button>
              </Link>
            </div>
          )}
