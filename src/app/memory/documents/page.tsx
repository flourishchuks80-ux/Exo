...
export default function DocumentsPage() {
  ...
  const { data: documents, isLoading } = useDocuments(walletAddress, masterKey);
  const deleteMutation = useDeleteMemory(walletAddress);

  const filtered = (documents ?? []).filter((d) =>
    search
      ? d.payload.title.toLowerCase().includes(search.toLowerCase()) ||
        d.payload.summary.toLowerCase().includes(search.toLowerCase())
      : true
  );
      ...
      <main className="pt-14 lg:pl-56">
        <div className="max-w-5xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-[#F0F4FF]">Documents</h1>
              <p className="text-sm text-[#8B9CC8] mt-1">
                {documents?.length ?? 0} documents stored on Arkiv Braga
              </p>
            </div>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              <FileText className="w-10 h-10 text-[#4F5E7A] mb-4" />
              <p className="text-[#8B9CC8] text-sm mb-4">No documents yet.</p>
              <p className="text-xs text-[#4F5E7A] mb-6">
                Upload documents or paste text to give your AI long-term reference material.
              </p>
              <Link href="/memory/new">
